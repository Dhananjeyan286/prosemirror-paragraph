(function () {
  ...
  */
  let DOMParser$1 = class DOMParser {
      ...
      */
      parse(dom, options = {}) {
          let context = new ParseContext$1(this, options, false);
      ...
      */
      parseSlice(dom, options = {}) {
          let context = new ParseContext$1(this, options, true);
          context.addAll(dom, options.from, options.to);
  ...
  const listTags$1 = { ol: true, ul: true };
  // Using a bitfield for node context options
  const OPT_PRESERVE_WS$1 = 1, OPT_PRESERVE_WS_FULL$1 = 2, OPT_OPEN_LEFT$1 = 4;
  function wsOptionsFor$1(type, preserveWhitespace, base) {
      if (preserveWhitespace != null)
          return (preserveWhitespace ? OPT_PRESERVE_WS$1 : 0) |
              (preserveWhitespace === "full" ? OPT_PRESERVE_WS_FULL$1 : 0);
      return type && type.whitespace == "pre" ? OPT_PRESERVE_WS$1 | OPT_PRESERVE_WS_FULL$1 : base & ~OPT_OPEN_LEFT$1;
  }
  let NodeContext$1 = class NodeContext {
  ...
  let ParseContext$1 = class ParseContext {
      ...
      options, isOpen) {
          ...
          let topOptions = wsOptionsFor$1(null, options.preserveWhitespace, 0) | (isOpen ? OPT_OPEN_LEFT$1 : 0);
          if (topNode)
      ...
      withStyleRules(dom, f) {
          ...
          if (!style)
              return f();
          let marks = this.readStyles(parseStyles$1(style));
          if (!marks)
      ...
      enterInner(type, attrs = null, solid = false, preserveWS) {
          ...
          let options = wsOptionsFor$1(type, preserveWS, top.options);
      ...
      addPendingMark(mark) {
          let found = findSameMarkInSet$1(mark, this.top.pendingMarks);
          if (found)
  ...
  */
  let DOMSerializer$1 = class DOMSerializer {
      /**
      ...
      */
      serializeNodeInner(node, options) {
          let { dom, contentDOM } = DOMSerializer.renderSpec(doc$3(options), this.nodes[node.type.name](node));
          if (contentDOM) {
              if (node.isLeaf)
      ...
      */
      static nodesFromSchema(schema) {
          let result = gatherToDOM$1(schema.nodes);
          if (!result.text)
  ...
  function setBlockType$1(tr, from, to, type, attrs) {
      ...
      tr.doc.nodesBetween(from, to, (node, pos) => {
          if (node.isTextblock && !node.hasMarkup(type, attrs) && canChangeType(tr.doc, tr.mapping.slice(mapFrom).map(pos), type)) {
              ...
              tr.step(new ReplaceAroundStep$1(startM, endM, startM + 1, endM - 1, new Slice$1(Fragment$1.from(type.create(attrs, null, node.marks)), 0, 0), 1, true));
              return false;
          }
      });
  }
  ...
  */
  ...
  function join(tr, pos, depth) {
      let step = new ReplaceStep$1(pos - depth, pos + depth, Slice$1.empty, true);
      tr.step(step);
  }
  /**
  ...
  */
  function dropPoint$1(doc, pos, slice) {
      let $pos = doc.resolve(pos);
  ...
  */
  function replaceStep$1(doc, from, to = from, slice = Slice$1.empty) {
  ...
  let Fitter$1 = class Fitter {
      ...
      openMore() {
          ...
          let inner = contentAt$1(content, openStart);
      ...
      dropNode() {
          ...
          if (inner.childCount <= 1 && openStart > 0) {
              ...
              this.unplaced = new Slice$1(dropFromFragment$1(content, openStart - 1, 1), openStart - 1, openAtEnd ? openStart - 1 : openEnd);
          }
          else {
      ...
      placeNodes({ sliceDepth, frontierDepth, parent, inject, wrap }) {
          ...
          while (taken < fragment.childCount) {
              ...
              if (taken > 1 || openStart == 0 || next.content.size) { // Drop empty open nodes
                  match = matches;
                  add.push(closeNodeStart$1(next.mark(type.allowedMarks(next.marks)), taken == 1 ? openStart : 0, taken == fragment.childCount ? openEndCount : -1));
              }
          }
          ...
          this.unplaced = !toEnd ? new Slice$1(dropFromFragment$1(slice.content, sliceDepth, taken), slice.openStart, slice.openEnd)
              : sliceDepth == 0 ? Slice$1.empty
                  : new Slice$1(dropFromFragment$1(slice.content, sliceDepth - 1, 1), sliceDepth - 1, openEndCount < 0 ? slice.openEnd : sliceDepth - 1);
      }
      mustMoveInline() {
      ...
      findCloseLevel($to) {
          scan: for (let i = Math.min(this.depth, $to.depth); i >= 0; i--) {
              ...
              let fit = contentAfterFits$1($to, i, type, match, dropInner);
              if (!fit)
                  continue;
              for (let d = i - 1; d >= 0; d--) {
                  let { match, type } = this.frontier[d];
                  let matches = contentAfterFits$1($to, d, type, match, true);
      ...
      close($to) {
          ...
          if (close.fit.childCount)
              this.placed = addToFragment$1(this.placed, close.depth, close.fit);
          $to = close.move;
      ...
      closeFrontierNode() {
          ...
          if (add.childCount)
              this.placed = addToFragment$1(this.placed, this.frontier.length, add);
      }
  };
  ...
  function addToFragment$1(fragment, depth, content) {
      ...
      return fragment.replaceChild(fragment.childCount - 1, fragment.lastChild.copy(addToFragment$1(fragment.lastChild.content, depth - 1, content)));
  }
  ...
  class Transform {
      ...
      */
      replace(from, to = from, slice = Slice$1.empty) {
          let step = replaceStep$1(this.doc, from, to, slice);
          if (step)
              this.step(step);
          return this;
      }
  ...
  */
  ...
  function scanFor$1(node, off, targetNode, targetOff, dir) {
      for (;;) {
          ...
          if (off == (dir < 0 ? 0 : nodeSize$1(node))) {
              ...
              if (!parent || parent.nodeType != 1 || hasBlockDesc$1(node) || atomElements$1.test(node.nodeName) ||
  ...
  function isOnEdge$1(node, offset, parent) {
      for (let atStart = offset == 0, atEnd = offset == nodeSize$1(node); atStart || atEnd;) {
          if (node == parent)
              return true;
  ...
  const gecko$1 = !ie$2 && /gecko\/(\d+)/i.test(agent$1);
  ...
  const safari$1 = !ie$2 && !!nav$1 && /Apple Computer/.test(nav$1.vendor);
  // Is true for both iOS and iPadOS for convenience
  const ios$1 = safari$1 && (/Mobile\/\w+/.test(agent$1) || !!nav$1 && nav$1.maxTouchPoints > 2);
  const mac$5 = ios$1 || (nav$1 ? /Mac/.test(nav$1.platform) : false);
  const windows$1 = nav$1 ? /Win/.test(nav$1.platform) : false;
  const android$1 = /Android \d/.test(agent$1);
  const webkit$1 = !!doc$2 && "webkitFontSmoothing" in doc$2.documentElement.style;
  ...
  function coordsAtPos(view, pos, side) {
      ...
      if (atom == null && offset && (side < 0 || offset == nodeSize$1(node))) {
          let before = node.childNodes[offset - 1];
          let target = before.nodeType == 3 ? textRange(before, nodeSize$1(before) - (supportEmptyRange ? 0 : 1))
  ...
  class NodeViewDesc extends ViewDesc {
      ...
      static create(parent, node, outerDeco, innerDeco, view, pos) {
          ...
          if (node.isText) {
              ...
              ({ dom, contentDOM } = DOMSerializer$1.renderSpec(document, node.type.spec.toDOM(node)));
          }
  ...
  function selectionFromDOM$1(view, origin = null) {
      ...
      if (selectionCollapsed$1(domSel)) {
          ...
          if (nearestDesc && nearestDescNode.isAtom && NodeSelection$1.isSelectable(nearestDescNode) && nearestDesc.parent
              && !(nearestDescNode.isInline && isOnEdge$1(domSel.focusNode, domSel.focusOffset, nearestDesc.dom))) {
  ...
  function selectionToDOM$1(view, force = false) {
      ...
      if (view.cursorWrapper) {
          ...
          if (brokenSelectBetweenUneditable$1 && !(sel instanceof TextSelection$1)) {
  ...
  const brokenSelectBetweenUneditable$1 = safari$1 || chrome$1 && chrome_version$1 < 63;
  ...
  function selectHorizontally$1(view, dir, mods) {
      ...
      if (sel instanceof TextSelection$1) {
          if (mods.indexOf("s") > -1) {
          ...
          else if (!(mac$5 && mods.indexOf("m") > -1)) {
              ...
              if (NodeSelection$1.isSelectable(node)) {
                  return apply$1(view, new NodeSelection$1(dir < 0 ? view.state.doc.resolve($head.pos - node.nodeSize) : $head));
              }
              else if (webkit$1) {
                  // Chrome and Safari will introduce extra pointless cursor
                  // positions around inline uneditable nodes, so we have to
                  // take over and move the cursor past them (#937)
                  return apply$1(view, new TextSelection$1(view.state.doc.resolve(dir < 0 ? nodePos : nodePos + node.nodeSize)));
              }
              else {
                  return false;
              }
  ...
  function isIgnorable$1(dom, dir) {
      let desc = dom.pmViewDesc;
      return desc && desc.size == 0 && (dir < 0 || dom.nextSibling || dom.nodeName != "BR");
  }
  function skipIgnoredNodes$1(view, dir) {
      return dir < 0 ? skipIgnoredNodesBefore$1(view) : skipIgnoredNodesAfter$1(view);
  }
  // Make sure the cursor isn't directly after one or more ignored
  // nodes, which will confuse the browser's cursor motion logic.
  function skipIgnoredNodesBefore$1(view) {
      let sel = view.domSelectionRange();
      ...
      if (gecko$1 && node.nodeType == 1 && offset < nodeLen$1(node) && isIgnorable$1(node.childNodes[offset], -1))
          force = true;
      for (;;) {
          if (offset > 0) {
              if (node.nodeType != 1) {
              ...
              while (prev && isIgnorable$1(prev, -1)) {
              ...
              if (!prev) {
                  ...
                  node = prev;
                  offset = nodeLen$1(node);
              }
          }
      }
      if (force)
          setSelFocus$1(view, node, offset);
      else if (moveNode)
          setSelFocus$1(view, moveNode, moveOffset);
  }
  // Make sure the cursor isn't directly before one or more ignored
  // nodes.
  function skipIgnoredNodesAfter$1(view) {
      let sel = view.domSelectionRange();
      let node = sel.focusNode, offset = sel.focusOffset;
      if (!node)
          return;
      ...
      for (;;) {
          if (offset < len) {
              ...
              while (next && isIgnorable$1(next, 1)) {
  ...
  function textNodeAfter$1(node, offset) {
      while (node && offset == node.childNodes.length && !hasBlockDesc$1(node)) {
  ...
  function textNodeBefore$1(node, offset) {
      while (node && !offset && !hasBlockDesc$1(node)) {
  ...
  function setSelFocus$1(view, node, offset) {
      if (node.nodeType != 3) {
          let before, after;
          if (after = textNodeAfter$1(node, offset)) {
              node = after;
              offset = 0;
          }
          else if (before = textNodeBefore$1(node, offset)) {
              node = before;
  ...
  function findDirection$1(view, pos) {
  ...
  function captureKeyDown$1(view, event) {
      ...
      if (code == 8 || (mac$5 && code == 72 && mods == "c")) { // Backspace, Ctrl-h on Mac
          return stopNativeHorizontalDelete$1(view, -1) || skipIgnoredNodes$1(view, -1);
      }
      else if ((code == 46 && !event.shiftKey) || (mac$5 && code == 68 && mods == "c")) { // Delete, Ctrl-d on Mac
          return stopNativeHorizontalDelete$1(view, 1) || skipIgnoredNodes$1(view, 1);
      }
      else if (code == 13 || code == 27) { // Enter, Esc
          return true;
      }
      else if (code == 37 || (mac$5 && code == 66 && mods == "c")) { // Left arrow, Ctrl-b on Mac
          let dir = code == 37 ? (findDirection$1(view, view.state.selection.from) == "ltr" ? -1 : 1) : -1;
          return selectHorizontally$1(view, dir, mods) || skipIgnoredNodes$1(view, dir);
      }
      else if (code == 39 || (mac$5 && code == 70 && mods == "c")) { // Right arrow, Ctrl-f on Mac
          let dir = code == 39 ? (findDirection$1(view, view.state.selection.from) == "ltr" ? 1 : -1) : 1;
          return selectHorizontally$1(view, dir, mods) || skipIgnoredNodes$1(view, dir);
      }
      else if (code == 38 || (mac$5 && code == 80 && mods == "c")) { // Up arrow, Ctrl-p on Mac
          return selectVertically$1(view, -1, mods) || skipIgnoredNodes$1(view, -1);
      }
      else if (code == 40 || (mac$5 && code == 78 && mods == "c")) { // Down arrow, Ctrl-n on Mac
          return safariDownArrowBug$1(view) || selectVertically$1(view, 1, mods) || skipIgnoredNodes$1(view, 1);
      }
      else if (mods == (mac$5 ? "m" : "c") &&
  ...
  function serializeForClipboard$1(view, slice) {
      ...
      let serializer = view.someProp("clipboardSerializer") || DOMSerializer$1.fromSchema(view.state.schema);
  ...
  function parseFromClipboard$1(view, text, html, plainText, $context) {
      ...
      if (asText) {
          ...
          if (parsed) {
              ...
              let { schema } = view.state, serializer = DOMSerializer$1.fromSchema(schema);
          ...
          slice = parser.parseSlice(dom, {
              ...
              ruleFromNode(dom) {
                  if (dom.nodeName == "BR" && !dom.nextSibling &&
                      dom.parentNode && !inlineParents$1.test(dom.parentNode.nodeName))
                      return { ignore: true };
                  return null;
              }
          });
      ...
      if (sliceData) {
          ...
          slice = Slice$1.maxOpen(normalizeSiblings$1(slice.content, $context), true);
  ...
  function normalizeSiblings$1(fragment, $context) {
      ...
      for (let d = $context.depth; d >= 0; d--) {
          ...
          fragment.forEach(node => {
              ...
              if (!wrap)
                  return result = null;
              if (inLast = result.length && lastWrap.length && addToSibling$1(wrap, lastWrap, node, result[result.length - 1], 0)) {
                  result[result.length - 1] = inLast;
              }
              else {
                  if (result.length)
                      result[result.length - 1] = closeRight$1(result[result.length - 1], lastWrap.length);
                  let wrapped = withWrappers$1(node, wrap);
                  result.push(wrapped);
  ...
  function withWrappers$1(node, wrap, from = 0) {
  ...
  function addToSibling$1(wrap, lastWrap, node, sibling, depth) {
      if (depth < wrap.length && depth < lastWrap.length && wrap[depth] == lastWrap[depth]) {
          let inner = addToSibling$1(wrap, lastWrap, node, sibling.lastChild, depth + 1);
          if (inner)
              return sibling.copy(sibling.content.replaceChild(sibling.childCount - 1, inner));
          let match = sibling.contentMatchAt(sibling.childCount);
          if (match.matchType(depth == wrap.length - 1 ? node.type : wrap[depth + 1]))
              return sibling.copy(sibling.content.append(Fragment$1.from(withWrappers$1(node, wrap, depth + 1))));
      }
  }
  ...
  function closeSlice$1(slice, openStart, openEnd) {
      if (openStart < slice.openStart)
          slice = new Slice$1(closeRange$1(slice.content, -1, openStart, slice.openStart, 0, slice.openEnd), openStart, slice.openEnd);
      if (openEnd < slice.openEnd)
          slice = new Slice$1(closeRange$1(slice.content, 1, openEnd, slice.openEnd, 0, 0), slice.openStart, openEnd);
      return slice;
  }
      ...
      for (let event in handlers$1) {
          ...
          view.dom.addEventListener(event, view.input.eventHandlers[event] = (event) => {
              if (eventBelongsToView(view, event) && !runCustomHandler(view, event) &&
                  (view.editable || !(event.type in editHandlers$1)))
                  handler(view, event);
      ...
      if (ios$1 && event.keyCode == 13 && !event.ctrlKey && !event.altKey && !event.metaKey) {
          ...
          view.input.lastIOSEnterFallbackTimeout = setTimeout(() => {
              if (view.input.lastIOSEnter == now) {
                  view.someProp("handleKeyDown", f => f(view, keyEvent$1(13, "Enter")));
                  view.input.lastIOSEnter = 0;
              }
          }, 200);
      }
      else if (view.someProp("handleKeyDown", f => f(view, event)) || captureKeyDown$1(view, event)) {
  ...
  editHandlers$1.keypress = (view, _event) => {
      let event = _event;
      if (inOrNearComposition$1(view, event) || !event.charCode ||
          event.ctrlKey && !event.altKey || mac$5 && event.metaKey)
          return;
  ...
  function eventCoords$1(event) { return { left: event.clientX, top: event.clientY }; }
  ...
  function selectClickedLeaf$1(view, inside) {
      ...
      if (node && node.isAtom && NodeSelection$1.isSelectable(node)) {
          updateSelection$1(view, new NodeSelection$1($pos), "pointer");
          return true;
      }
      return false;
  }
  function selectClickedNode$1(view, inside) {
      ...
      if (selectAt != null) {
          updateSelection$1(view, NodeSelection$1.create(view.state.doc, selectAt), "pointer");
          return true;
      }
      else {
          return false;
  ...
  function handleSingleClick$1(view, pos, inside, event, selectNode) {
      return runHandlerOnContext$1(view, "handleClickOn", pos, inside, event) ||
  ...
  function handleDoubleClick$1(view, pos, inside, event) {
      return runHandlerOnContext$1(view, "handleDoubleClickOn", pos, inside, event) ||
          view.someProp("handleDoubleClick", f => f(view, pos, event));
  }
  function handleTripleClick$1(view, pos, inside, event) {
      return runHandlerOnContext$1(view, "handleTripleClickOn", pos, inside, event) ||
  ...
  function defaultTripleClick$1(view, inside, event) {
      ...
      if (inside == -1) {
          if (doc.inlineContent) {
              updateSelection$1(view, TextSelection$1.create(doc, 0, doc.content.size), "pointer");
              return true;
          }
          return false;
      }
      ...
      for (let i = $pos.depth + 1; i > 0; i--) {
          ...
          if (node.inlineContent)
              updateSelection$1(view, TextSelection$1.create(doc, nodePos + 1, nodePos + 1 + node.content.size), "pointer");
          else if (NodeSelection$1.isSelectable(node))
              updateSelection$1(view, NodeSelection$1.create(doc, nodePos), "pointer");
          else
              continue;
          return true;
      }
  ...
  const selectNodeModifier$1 = mac$5 ? "metaKey" : "ctrlKey";
  handlers$1.mousedown = (view, _event) => {
      let event = _event;
      view.input.shiftKey = event.shiftKey;
      let flushed = forceDOMFlush$1(view);
      let now = Date.now(), type = "singleClick";
      if (now - view.input.lastClick.time < 500 && isNear$1(event, view.input.lastClick) && !event[selectNodeModifier$1]) {
  ...
  let MouseDown$1 = class MouseDown {
      ...
      up(event) {
          ...
          if (this.allowDefault || !pos) {
              ...
              updateSelection$1(this.view, Selection$1.near(this.view.state.doc.resolve(pos.pos)), "pointer");
              event.preventDefault();
          }
          else {
              setSelectionOrigin$1(this.view, "pointer");
  ...
  const timeoutComposition$1 = android$1 ? 5000 : -1;
  ...
  */
  ...
  const brokenClipboardAPI$1 = (ie$2 && ie_version$1 < 15) ||
  ...
  function doPaste$1(view, text, html, preferPlain, event) {
      let slice = parseFromClipboard$1(view, text, html, preferPlain, view.state.selection.$from);
  ...
  editHandlers$1.paste = (view, _event) => {
      ...
      let data = brokenClipboardAPI$1 ? null : event.clipboardData;
      let plain = view.input.shiftKey && view.input.lastKeyCode != 45;
      if (data && doPaste$1(view, getText$1(data), data.getData("text/html"), plain, event))
          event.preventDefault();
      else
  ...
  const dragCopyModifier$1 = mac$5 ? "altKey" : "ctrlKey";
  handlers$1.dragstart = (view, _event) => {
      ...
      let slice = (node || view.state.selection).content(), { dom, text } = serializeForClipboard$1(view, slice);
      ...
      view.dragging = new Dragging$1(slice, !event[dragCopyModifier$1], node);
  };
  ...
  editHandlers$1.dragover = editHandlers$1.dragenter = (_, e) => e.preventDefault();
  editHandlers$1.drop = (view, _event) => {
      ...
      if (slice) {
          ...
          slice = parseFromClipboard$1(view, getText$1(event.dataTransfer), brokenClipboardAPI$1 ? null : event.dataTransfer.getData("text/html"), false, $mouse);
      }
      ...
      if (!slice)
          return;
      event.preventDefault();
      let insertPos = slice ? dropPoint$1(view.state.doc, $mouse.pos, slice) : $mouse.pos;
      if (insertPos == null)
          insertPos = $mouse.pos;
      let tr = view.state.tr;
      if (move) {
      ...
      if (isNode && NodeSelection$1.isSelectable(slice.content.firstChild) &&
          ...
          tr.setSelection(selectionBetween$1(view, $pos, tr.doc.resolve(end)));
      }
      view.focus();
  ...
  handlers$1.beforeinput = (view, _event) => {
      ...
      if (chrome$1 && android$1 && event.inputType == "deleteContentBackward") {
          ...
          setTimeout(() => {
              ...
              view.dom.blur();
              view.focus();
              if (view.someProp("handleKeyDown", f => f(view, keyEvent$1(8, "Backspace"))))
                  return;
          ...
          return this == other ||
              (other instanceof WidgetType &&
                  (this.spec.key && this.spec.key == other.spec.key ||
                      this.toDOM == other.toDOM && compareObjs$1(this.spec, other.spec)));
      }
      destroy(node) {
          if (this.spec.destroy)
  ...
  let InlineType$1 = class InlineType {
      ...
      eq(other) {
          return this == other ||
              (other instanceof InlineType && compareObjs$1(this.attrs, other.attrs) &&
                  compareObjs$1(this.spec, other.spec));
      }
  ...
  let NodeType$1 = class NodeType {
      ...
      eq(other) {
          return this == other ||
              (other instanceof NodeType && compareObjs$1(this.attrs, other.attrs) &&
                  compareObjs$1(this.spec, other.spec));
      }
      destroy() { }
  };
  ...
  */
  let Decoration$1 = class Decoration {
      /**
      @internal
      */
      constructor(
      ...
      */
      static widget(pos, toDOM, spec) {
          return new Decoration(pos, pos, new WidgetType$1(toDOM, spec));
      }
      /**
      ...
      */
      static inline(from, to, attrs, spec) {
          return new Decoration(from, to, new InlineType$1(attrs, spec));
      }
      /**
      ...
      */
      ...
      @internal
      */
      get inline() { return this.type instanceof InlineType$1; }
      /**
      @internal
      */
      get widget() { return this.type instanceof WidgetType$1; }
  };
  ...
  */
  let DecorationSet$1 = class DecorationSet {
      /**
      @internal
      */
      ...
      */
      static create(doc, decorations) {
          return decorations.length ? buildTree$1(decorations, doc, 0, noSpec$1) : empty$1;
      }
      /**
      ...
      */
      map(mapping, doc, options) {
          if (this == empty$1 || mapping.maps.length == 0)
              return this;
          return this.mapInner(mapping, doc, 0, 0, options || noSpec$1);
      }
      /**
      @internal
      */
      mapInner(mapping, node, offset, oldOffset, options) {
          ...
          if (this.children.length)
              return mapChildren$1(this.children, newLocal || [], mapping, node, offset, oldOffset, options);
          else
              return newLocal ? new DecorationSet(newLocal.sort(byPos$1), none$1) : empty$1;
      }
      /**
      ...
      */
      ...
      addInner(doc, decorations, offset) {
          ...
          doc.forEach((childNode, childOffset) => {
              ...
              if (!(found = takeSpansForNode$1(decorations, childNode, baseOffset)))
                  return;
              if (!children)
              ...
              if (children[childIndex] == childOffset)
                  ...
                  children.splice(childIndex, 0, childOffset, childOffset + childNode.nodeSize, buildTree$1(found, childNode, baseOffset + 1, noSpec$1));
              childIndex += 3;
          });
          let local = moveSpans$1(childIndex ? withoutNulls$1(decorations) : decorations, -offset);
          for (let i = 0; i < local.length; i++)
              if (!local[i].type.valid(doc, local[i]))
                  local.splice(i--, 1);
          return new DecorationSet(local.length ? this.local.concat(local).sort(byPos$1) : this.local, children || this.children);
      }
      /**
      ...
      */
      forChild(offset, node) {
          ...
          for (let i = 0; i < this.local.length; i++) {
              ...
              if (dec.from < end && dec.to > start && (dec.type instanceof InlineType$1)) {
          ...
          if (local) {
              let localSet = new DecorationSet(local.sort(byPos$1), none$1);
  ...
  */
  ...
  let DecorationGroup$1 = class DecorationGroup {
      constructor(members) {
          this.members = members;
      }
      map(mapping, doc) {
          const mappedDecos = this.members.map(member => member.map(mapping, doc, noSpec$1));
      ...
      static from(members) {
          switch (members.length) {
              ...
              default: return new DecorationGroup(members.every(m => m instanceof DecorationSet$1) ? members :
                  members.reduce((r, m) => r.concat(m instanceof DecorationSet$1 ? m : m.members), []));
          }
      }
  };
  function mapChildren$1(oldChildren, newLocal, mapping, node, offset, oldOffset, options) {
      ...
      if (mustRebuild) {
          let decorations = mapAndGatherRemainingDecorations$1(children, oldChildren, newLocal, mapping, offset, oldOffset, options);
  ...
  function moveSpans$1(spans, offset) {
  ...
  function mapAndGatherRemainingDecorations$1(children, oldChildren, decorations, mapping, offset, oldOffset, options) {
  ...
  function takeSpansForNode$1(spans, node, offset) {
      if (node.isLeaf)
          return null;
  ...
  function buildTree$1(spans, node, offset, options) {
      ...
      node.forEach((childNode, localStart) => {
          let found = takeSpansForNode$1(spans, childNode, localStart + offset);
          if (found) {
              hasNulls = true;
              let subtree = buildTree$1(found, childNode, offset + localStart + 1, options);
              if (subtree != empty$1)
                  children.push(localStart, localStart + childNode.nodeSize, subtree);
          }
      });
      let locals = moveSpans$1(hasNulls ? withoutNulls$1(spans) : spans, -offset).sort(byPos$1);
  ...
  function viewDecorations(view) {
      ...
      if (view.cursorWrapper)
          found.push(DecorationSet$1.create(view.state.doc, [view.cursorWrapper.deco]));
      return DecorationGroup$1.from(found);
  }
  ...
  class DOMObserver {
      constructor(view, handleDOMChange) {
          ...
          this.observer = window.MutationObserver &&
              new window.MutationObserver(mutations => {
                  ...
                  if (ie$2 && ie_version$1 <= 11 && mutations.some(m => m.type == "childList" && m.removedNodes.length ||
      ...
      flush() {
          ...
          if (from < 0 && newSel && view.input.lastFocus > Date.now() - 200 &&
              ...
              selectionCollapsed$1(sel) && (readSel = selectionFromDOM$1(view)) &&
  ...
  function readDOMChange(view, from, to, typeOver, addedNodes) {
      ...
      if (ie$2 && ie_version$1 <= 11 && change.endB == change.start + 1 &&
      ...
      if (view.state.selection.anchor > change.start &&
          ...
          view.someProp("handleKeyDown", f => f(view, keyEvent$1(8, "Backspace")))) {
      ...
      if (android$1 && !inlineChange && $from.start() != $to.start() && $to.parentOffset == 0 && $from.depth == $to.depth &&
          ...
          setTimeout(() => {
              view.someProp("handleKeyDown", function (f) { return f(view, keyEvent$1(13, "Enter")); });
          }, 20);
      }
  ...
  function resolveSelection(view, doc, parsedSel) {
      if (Math.max(parsedSel.anchor, parsedSel.head) > doc.content.size)
          return null;
      return selectionBetween$1(view, doc.resolve(parsedSel.anchor), doc.resolve(parsedSel.head));
  }
  ...
  class EditorView {
      ...
      */
      ...
      updateStateInner(state, prevProps) {
          ...
          if (updateSel) {
              ...
              // #1011, #1013, #1035).
              let forceSelUpdate = updateDoc && (ie$2 || chrome$1) && !this.composing &&
      ...
      */
      ...
      updateDraggedNode(dragging, prev) {
          ...
          this.dragging = new Dragging$1(dragging.slice, dragging.move, found < 0 ? undefined : NodeSelection$1.create(this.state.doc, found));
      }
      someProp(propName, f) {
  ...
  */
  function addListNodes(nodes, itemContent, listGroup) {
      return nodes.append({
          ...
          list_item: add$1(listItem$1, { content: itemContent })
      });
  }
  ...
  function keyName(event) {
    ...
    var ignoreKey = mac$4 && event.metaKey && event.shiftKey && !event.ctrlKey && !event.altKey || ie$1 && event.shiftKey && event.key && event.key.length == 1 || event.key == "Unidentified";
  ...
  function normalizeKeyName(name) {
    ...
    for (let i = 0; i < parts.length - 1; i++) {
      ...
      if (/^(cmd|meta|m)$/i.test(mod)) meta = true;else if (/^a(lt)?$/i.test(mod)) alt = true;else if (/^(c|ctrl|control)$/i.test(mod)) ctrl = true;else if (/^s(hift)?$/i.test(mod)) shift = true;else if (/^mod$/i.test(mod)) {
        if (mac$3) meta = true;else ctrl = true;
  ...
  */
  ...
  editHandlers.keypress = (view, _event) => {
    let event = _event;
    if (inOrNearComposition(view, event) || !event.charCode || event.ctrlKey && !event.altKey || mac$1 && event.metaKey) return;
    if (view.someProp("handleKeyPress", f => f(view, event))) {
      event.preventDefault();
      return;
    }
  ...
  */
  ...
  class MenuBarView {
    constructor(editorView, options) {
      ...
      this.wrapper = crelt("div", {
        class: prefix$3 + "-wrapper"
      });
  ...
  */
  function buildMenuItems(schema) {
    ...
    if (node = schema.nodes.blockquote) r.wrapBlockQuote = wrapItem(node, {
      title: "Wrap in block quote",
      icon: icons.blockquote
    });
    if (node = schema.nodes.paragraph) r.makeParagraph = blockTypeItem(node, {
      title: "Change to paragraph",
      label: "Plain"
    });
    if (node = schema.nodes.code_block) r.makeCodeBlock = blockTypeItem(node, {
      title: "Change to code block",
      label: "Code"
    });
    if (node = schema.nodes.heading) for (let i = 1; i <= 10; i++) r["makeHead" + i] = blockTypeItem(node, {
  ...
  */
  ...
  function getSubConfig(config, item) {
  ...
  function isSubConfigPresent(config, name) {
    return config.find(subConfig => {
      return subConfig.name === name;
    });
  }
  function constructAndGetConfig(config) {
    if (!config) {
  ...
  function getTextALingmentMenu(textAlignmentConfig, setTextAlignment) {
    ...
    return new Dropdown([leftAlignBtn, rightAlignBtn, centerAlignBtn, justifyAlignBtn], {
      label: "Text-Align"
    });
  }
  function getTextDirectionMenu(textDirectionConfig, setTextDirection) {
    ...
    return new Dropdown([ltrBtn, rtlBtn], {
  ...
  function getLineHeightMenu(lineHeightConfig, setLineHeight) {
    ...
    return new Dropdown([singleBtn, normalBtn, singleAndHalfBtn, doubleBtn], {
      label: "Line-height"
    });
  }
  function getMenuBar(options) {
  ...
  function splitBlockKeepAttrsAndMarks(state, dispatch) {
  ...
  function getEnterHandlingPlugin(schema) {
    var key = {};
    key.Enter = enterHandler(schema);
    return keymap(key);
  }

  function getParagraphNode(config) {
    config = constructAndGetConfig(config);
    var paragraphNode = {
      content: "inline*",
      // no i18n
  ...
  function addParagraphNode(nodes, config) {
  ...
  function getPlugins(options) {
    options.config = constructAndGetConfig(options.config);
    let plugins = [buildInputRules(options.schema), getEnterHandlingPlugin(options.schema), keymap(buildKeymap(options.schema, options.mapKeys)), keymap(baseKeymap), dropCursor(), gapCursor()];
